...
resource "azurerm_virtual_machine_extension" "start-apache-server" {
  name                 = "hostname"
  virtual_machine_id   = azurerm_linux_virtual_machine.vm01-spk01.id
...
resource "azurerm_public_ip" "vm01-spk01-pub-ip" {
  name                = "vm01-spk01-pub-ip-${var.product-name}"
